...
const api = {
  key: 'e63b8f285597e55b74d134459cbd51a7',
  incomeStatement:
...
function App() {
  ...
  function handleKey(e) {
    if (e.key === 'Enter') {
      let dcfData = fetch(`${api.dcf}${ticker}/apikey=${api.key}`);
      let incomeData = fetch(
        `${api.incomeStatement}${ticker}/apikey=${api.key}`
      );
      let balanceData = fetch(`${api.balanceSheet}${ticker}/apikey=${api.key}`);
      let profile = fetch(`${api.profile}${ticker}/apikey=${api.key}`);
      let cashFlow = fetch(`${api.cashFlow}${ticker}/apikey=${api.key}`);
